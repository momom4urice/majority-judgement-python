...
if __name__ == "__main__":
    # Load CSV data
    with open('RTM Name.csv', 'rb') as csvfile:
    ...
    for votes in votes_list:
        print(votes)
    n_votes = votes_list[0].n_votes

    # Plot in alphabetical order
    sns.set()
    plot_mj("fig/results_alpha.png", mention_map, votes_dict_mention, candidate_map)

    # Plot in MJ order
    candidate_order_mj = [candidate_map.inv[votes.name]
                          for votes in votes_list]
    plot_mj("fig/results_mj.png", mention_map, votes_dict_mention,
            candidate_map, candidate_order=candidate_order_mj)
